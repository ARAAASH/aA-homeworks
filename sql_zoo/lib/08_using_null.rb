...
def null_dept
  # List the teachers who have NULL for their 
  #department.
  execute(<<-SQL)
    SELECT
      name
...
def all_teachers_join
  # Use a type of JOIN that will list all teachers 
...
def all_depts_join
  # Use a different JOIN so that all departments are 
  #listed.
  # NB: you can avoid RIGHT OUTER JOIN 
  #(and just use LEFT) by swapping
...
def teachers_and_mobiles
  ...
  #Use the number '07986
  # 444 2266' if no number is given. 
  #Show teacher name and mobile
...
def teachers_and_depts
  ...
  #to print the teacher name and
  # department name. Use the string 'None' 
  #where there is no
  # department.
  execute(<<-SQL)
    SELECT 
...
def num_teachers_and_mobiles
  # Use COUNT to show the number of teachers and 
  #the number of
  # mobile phones.
...
def dept_staff_counts
  # Use COUNT and GROUP BY dept.name to show each 
  #department and
  # the number of staff. Structure your JOIN to 
  #ensure that the
...
def teachers_and_divisions
  ...
  #followed by 'Sci' if
  # the the teacher is in dept 1 or 2 and 'Art' 
  #otherwise.
  execute(<<-SQL)
    SELECT
      name, 
...
def teachers_and_divisions_two
  # Use CASE to show the name of each teacher 
  #followed by 'Sci' if
  # the teacher is in dept 1 or 2, 'Art' if the 
  #dept is 3, and
  # 'None' otherwise.
  execute(<<-SQL)
    SELECT
